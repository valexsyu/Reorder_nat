...
twccli mk ccs -n $CCS_NAME -gpu ${GPU_NUM}m -itype Custom\ Image -img pytorch-21.08-py3:CTCPLM1 -wait -json > $CCS_NAME.ccs_res.log
twccli ls ccs
CCS_ID=$(cat $CCS_NAME.ccs_res.log | jq '.id')
echo "==============CCS_ID:$CCS_ID========================="
