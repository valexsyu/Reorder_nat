...
def get_xlmr_sentence(tokens):
    sentence = []
    length = len(tokens)
    i=0
    while(i < length):
        index = [i]
        index = find_xlmr_pound_key(tokens, i, index)
        i = index[-1]+1
        word = [tokens[j].strip(" ") for j in index]
        word = "".join(word)
        sentence.append(word)
...
def find_xlmr_pound_key(tokens, i, index):
    if i == len(tokens)-1:
        return index
    if " " not in tokens[i+1]:
        return index
...
@dataclass
class XlmrBPEConfig(BertBPEConfig):
    pretrained_bpe: Optional[str] = field(
        default=None, metadata={"help": "pre-trained tgt bpe model"})
    pretrained_bpe_src: Optional[str] = field(
        default=None, metadata={"help": "pre-trained src bpe model"}
...
@register_bpe("xlmr", dataclass=XlmrBPEConfig)
class XlmrBPE(object):
    def __init__(self, cfg, if_src=False):
        try:
            from transformers import AutoTokenizer, BertTokenizer
        except ImportError:
