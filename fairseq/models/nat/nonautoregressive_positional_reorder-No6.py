...
class NATPositionalReorderModel(BaseFairseqModel):
    ...
    def forward(
        ...
        if self.reorder_translation == "translation" :
            ...
            alignments_upsample = upsampling(alignments, self.num_upsampling_rate)
            if self.use_pretrained_embedding :
                with torch.no_grad():
                    token_embeddings = get_pretrained_embedding(src_tokens)
                    translator_token_embedding = upsampling(token_embeddings, self.num_upsampling_rate)
            else:       
                token_embeddings = None 
                if self.pretrained_model_name is not None:
                    translator_token_embedding = self.translator_encoder.embeddings.word_embeddings(src_upsample_tokens)
                else:
                    translator_token_embedding = self.translator_encoder.sentence_encoder.embed_tokens(src_upsample_tokens) 

            if self.use_align_position :
                align_positions_embed = self.embed_align_positions(alignments_upsample)
                translator_token_embedding = translator_token_embedding + align_positions_embed
            ...
            reorder_position_embed_out = upsampling(logits, self.num_upsampling_rate)                                                           
            if self.use_align_position :
                align_positions_embed = self.embed_align_positions(alignments_upsample)
                #translator_token_embedding = translator_token_embedding + align_positions_embed   #test
                translator_token_embedding = translator_token_embedding + reorder_position_embed_out     
            if self.pretrained_model_name is not None:
                # logits, inner_state = self.translator_encoder.forward(input_ids = None, output_hidden_states=False, return_dict=False, 
                                            # inputs_embeds=translator_token_embedding) 
                trans_outputs = self.translator_encoder.forward(input_ids = None, output_hidden_states=True, return_dict=True, 
                                            inputs_embeds=translator_token_embedding)                                                
                logits = trans_outputs['logits']
                output_hidden_rep = trans_outputs['hidden_states'][-1]     
                # logits = self.output_layer(logits)        ##use bibert no lm 
            else:
                logits, inner_state = self.translator_encoder.forward(src_tokens = src_upsample_tokens, features_only=False, 
                                                return_all_hiddens=False, classification_head_name=None, 
                                                token_embeddings=translator_token_embedding,  **kwargs)  
                # output_hidden_rep = inner_state["inner_states"][-1].transpose(1,0)                                                                                   

            if self.lm_loss:
                with torch.no_grad():
                   tgt_output_rep=get_pretrained_embedding(tgt_tokens)
                lm_loss_output = compute_lm_loss(output_rep=output_hidden_rep, logits=logits, src_tokens=src_upsample_tokens, \
                                        tgt_tokens=tgt_tokens, tgt_output_rep=tgt_output_rep, \
                                        reduce=True)
                return {
                    "word_ins": {
                        "out": logits,
    ...
    def forward_inference(self, src_tokens, src_lengths, alignments, **kwargs):
        ...
        if self.pretrained_model_name is not None:
            trans_outputs = self.translator_encoder.forward(input_ids = None, output_hidden_states=False, return_dict=True, 
                                        inputs_embeds=translator_token_embedding)     
            logits = trans_outputs['logits']                                            
        else:
            logits, inner_state = self.translator_encoder.forward(src_tokens = src_upsample_tokens, features_only=False, 
