#---------Path Setting-------------------#
CHECKPOINT=checkpoints/No-3  #(for merger only)
DATA_BIN=data-bin/iwslt14.tokenized.de-en.distilled
MAX_TOKENS=6000  #4000 3090
MAX_EPOCH=1
CUR_START_EPOCH=300
CUDA_DEVICES=0
#---------Battleship Setting-------------#
...
cat > $CHECKPOINT/temp1.sh << 'endmsg'
    ...
    --pretrained-translation checkpoints/No-0-3-translation/checkpoint_last.pt \
    --pretrained-reorder checkpoints/No-1-5/checkpoint_last.pt \
...
#--encoder-causal-attn \
#--update-freq 4 \
#--wandb-project ReorderNAT-No-2-5 \
endmsg
