...
function get_dataset() {
    ...
    then
        dataset="wmt16_ro_en_bt_mbert" 
    elif [ "$i" = "M_BT" ]
    then
        dataset="wmt16_en_ro_bt_mbert"    
    elif [ "$i" = "X" ]
    then
...
VALID_ARGS=$(getopt -o e:,b: --long experiment:,twcc,batch-size:,cpu,data-subset:,debug,load-exist-bleu,ck-types:,avg-ck-turnoff,no-atten-mask,skip-exist-genfile,avg-speed:,ctc-beam-decoding,beam-size:,kenlm-path:,alpha:,beta:,fp16 -- "$@")
if [[ $? -ne 0 ]]; then
    exit 1;
fi
...
# csv_file=call_scripts/generate/output_file/output_read${no_atten_postfix}_beam.csv
if [ -f "$csv_file" ]; then 
    rm $csv_file
fi
...
for i in "${!exp_array[@]}"; do 
    ...
    if [ -d "$CHECKPOINT" ]; then
        ...
        bleu_array=()
        speed_avg_array=()
        csv_file=call_scripts/generate/output_file/output_read_${experiment_id}_${no_atten_postfix}_beam.csv
        for data_type in "${data_subset[@]}" ; do
            output_bleu_array=()
            output_speed_avg=()
            for ck_ch in "${ck_types[@]}"; do
